...
<?php
      // Вывести элементы массива на страницу 
  $programming = ['js', 'ruby', 'PHP', 'java'];
      // Добавить в массив элемент 
  $programming[] = 'pyton';
  foreach($programming as $lang) {
    echo $lang . '</br>';
  }

    // Посчитать сколько элементом в массиве
  $c = count($programming);
  echo $c;

    // Задача! Выведем те числа, которые делятся на 5
  $arr5 = [40, 61, 500, 1, 32, 34, 5, 3000];
...
<!-- Недавно искал работу, около 10 собеседований прошел. Ничего даже близко похожего не было. Разве что один раз спросили про get/post. Зато вообще везде были вопросы на sql, обязательно по join таблиц, везде спрашивали про какие join-ы бывают и чем отличаются. И в больших компаниях зачастую давали абсолютно идентичные задачи решать, как будто какой-то типовой тест где-то нашли и юзают его.

Приведу тест, который встречался мне аж три раза и все в больших, серьезных компаниях, в частности amocrm и qsoft

1. Написать функцию, которая в зависимости от переданного числа просклоняет слово программист и выведет строку (1 программист, 2 программиста и тд). Надо чтобы любое число склонялось правильно

2. Есть компании, у которых могут быть офисы в разных городах. Надо составить структуру таблиц, и написать запросы чтобы вывести офисы каких компаний есть в городе, в каких городах есть офисы одной компании ну и тд

3.Есть простейший код примерно такой
<?php
for ($i=0;$i<5;$i++) :
     $n = 1;
    //  echo “$n,”;
...
?>
Напиши че он выведет

4.По js грубо говоря есть форма с двумя инпутами и сабмит. Надо написать код, который при отправке формы выведет сумму цифр в полях в какую-то строку вне формы

5.хз получится ли нормально описать. В общем есть многоуровневый массив , в котором лежат элементы списка типа array(0=>1,1=>array(2.1,2.2,2.3),2=>3 и тд). Надо написать функцию, которая рекурсивно обойдет массив и выведет все значения в виде ну типа как многоуровневый список в ворде, ну то есть отступы там и все такое
1.
2.
    2.1.
    2.2.
...
Такой тест с незначительными вариациями встретил 3 раза из десяти.
Вообще кажется, что все очень просто, но обычно писать все это добро надо примерно за 6 минут на каждую задачу на листке бумаги и в связи с этим надо быть оч внимательным, чтобы не накосячить в мелочах

Из допвопросов самые сложные это разного рода хитрожопые джоины таблиц. Часто спрашивали про безопасность в sql запросах. Советую почитать про как делаются sql инъекции и все такое. 
Были вопросы на работу с датой и временем, сортировку многоуровневых массивов, распарсить url на домен и параметры. Ну и бывало про ООП, как код структурируешь, какие впечатления от какого-то фреймворка, какую ide юзаешь (хотят обычно storm), могешь ли в линукс (кстати очень советую мочь) -->